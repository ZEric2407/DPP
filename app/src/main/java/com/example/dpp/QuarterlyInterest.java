...
public class QuarterlyInterest extends Interest{
    public QuarterlyInterest(String name, int discRate, double debt, Calendar pmtDate, double initialDebt, @Nullable Calendar debtStart){
        ...
        if (debtStart == null) {
            ...
            this.debtStart.add(Calendar.MONTH, -3);
        } else {
    ...
    public double declareCF(double amt, Calendar date) {
        ...
        lastPayment.add(Calendar.MONTH, -3);
        int count = 0;
        while (CFDate.compareTo(lastPayment) <= 0){
            CFDate.add(Calendar.MONTH, 3);
            count++;
        }
    ...
    public double computeEIR(){
        return Math.pow((this.discRate/100.0 + 1), 4) - 1;
    }
}
